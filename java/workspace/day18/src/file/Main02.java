...
public class Main02 {
	...
	public static void main(String[] args) {
		// 저장할 파일의 경로
		final String PATH = "C:\\test\\lsh_test.txt";
		...
		// fianlly 블록에서 인식하기 위해서 선언부를 위로 이동시킨다. 위의 buffer와 마차가지
		OutputStream out = null;
		// 파일 저장 절차 시작
		try {
			// 파일 저장을위한 FileOutputStream 클래스의 객체를 생성할 떄는 사용할 수 없는 경로에 대한
			// 지정을 방지하기 위하여 예외처리가 강제된다.
			...
			e.printStackTrace();
			System.out.println("[ERROR] 알수 없는 에러  >> " + PATH);
		} 
		finally {try {
			out.close();
		} catch (IOException e) {
